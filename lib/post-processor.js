...
const processors = {
  ...
  singlePost: require('./processors/base-processor')('.single_post'),
...
function process(post, body) {
  ...
  } else if (linkMatches(post, 'blog.openshift.com')) {
    return processors.mainTag(post, body);
  } else if (linkMatches(post, 'www.howardism.org')) {
    return processors.contentId(post, body);
  } else if (linkMatches(post, 'techieme.in')) {
    return processors.singlePost(post, body);
  } else if (linkMatches(post, 'alvinalexander.com')) {
    return processors.contentId(post, body);
